...
CREATE TABLE Bug (id BIGSERIAL NOT NULL,userId BIGINT NOT NULL,title VARCHAR(100) NOT NULL, description TEXT NOT NULL, creationDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,imageURL VARCHAR, status varchar(20) check(status in ('SOLVED', 'IN PROGRESS', 'NOT SOLVED')) NOT NULL DEFAULT 'NOT SOLVED', voteCount INT DEFAULT 0, PRIMARY KEY(id),FOREIGN KEY(userId) REFERENCES Users(id) ON DELETE CASCADE);
...
CREATE TABLE BugTag ( bugId BIGINT NOT NULL,tagId BIGINT NOT NULL, PRIMARY KEY (bugId, tagId), FOREIGN KEY (bugId) REFERENCES Bug(id) ON DELETE CASCADE, FOREIGN KEY (tagId) REFERENCES Tag(id) ON DELETE CASCADE);
